...
parser.add_argument('--videos', type=Path, default='./dataset')
...
reid_model = torch.hub.load('XingangPan/IBN-Net', 'resnet50_ibn_a', pretrained=True).cuda()
# reid_model = reid_model.to('cpu')
reid_model.eval()
...
for video_paths in sorted(list(args.videos.glob('*.imgs'))):
    ...
    embs = torch.zeros(len(dets), 1000)
